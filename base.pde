...

{
    ...
    {
      Complex wInc = new Complex(cos(PI/inc),-sin(PI/inc));  
...
// Compute NxN spatialSignal (array of floats) from NxN frequencySignal (array of complex).
//
...

{
    ...
    for (r=0;r<N;r++){
       for(c=0;c<N;c++){
          frequencySignal[c+r*N]=new Complex(spatialSignal[c+r*N]*pow(-1, r+c),0.0); 
       }
    }
...

{
  ...
  int c;
  int i;
    //get complex conjugate by making imaginary component negative
    for (i=0; i<N*N; i++){
      frequencySignal[i]= new Complex(frequencySignal[i].real,-frequencySignal[i].imag);
    }
    FFT2D(frequencySignal,N);
    // Apply 2D F.T.
    //copy resulting frequency signal into spatial signal variable
    for (i=0; i<N*N; i++){
      spatialSignal[i]=frequencySignal[i].real;
    }
    //normalize by diving by N^2
    for (i=0; i<N*N; i++) {
      spatialSignal[i]=spatialSignal[i]/pow(N,2);
    }
    //de-centre by dividing by (-1)^(row+col)
    for (r=0; r<N; r++){
        for (c=0; c<N; c++){
            spatialSignal[c+r*N] = spatialSignal[c+r*N]/pow(-1,r+c);
        }
    }
}
